...
// clear costmap
void CostmapLayer::clearAreaAll()
{
  unsigned char* grid = getCharMap();
  for(int x=0; x<(int)getSizeInCellsX(); x++){
    for(int y=0; y<(int)getSizeInCellsY(); y++){
      ...
      if(grid[index]==LETHAL_OBSTACLE){
        //ROS_WARN("grid[index] = %d",grid[index]);
        //ROS_WARN("clearAreaAll - obstacle to free space");
        grid[index] = FREE_SPACE; // default value is FREE_SPACE if track_unknown_space is false
      }
    }
  }
}
...
void CostmapLayer::updateWithMax(am_costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
...
void CostmapLayer::updateWithTrueOverwrite(am_costmap_2d::Costmap2D& master_grid, int min_i, int min_j,
                                           int max_i, int max_j)
{
  if (!enabled_)
    return;
...
void CostmapLayer::updateWithOverwrite(am_costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
  unsigned char* master = master_grid.getCharMap();
...
void CostmapLayer::updateWithAddition(am_costmap_2d::Costmap2D& master_grid, int min_i, int min_j, int max_i, int max_j)
{
  if (!enabled_)
    return;
  unsigned char* master_array = master_grid.getCharMap();
  ...
  for (int j = min_j; j < max_j; j++)
  {
    ...
    for (int i = min_i; i < max_i; i++)
    {
      ...
      else
      {
        ...
        if (sum >= am_costmap_2d::INSCRIBED_INFLATED_OBSTACLE)
            master_array[it] = am_costmap_2d::INSCRIBED_INFLATED_OBSTACLE - 1;
        else
            master_array[it] = sum;
      }
      it++;
